function CDices(iType) {
    ...
    this._init = function (iType) {
        ...
        _oDiceLaunch.gotoAndStop("stop");
        _oDiceLaunch.on("animationend", function(evt) {
            this._onDiceLaunchAnimationEnd();
        },this);
        
        _oDice2.gotoAndStop("stop1");
        _oDice2.on("animationend", function(evt) {
            this._onDiceResultAnimationEnd(evt.currentTarget.currentAnimation);
        },this);
        
    };

    this._onDiceLaunchAnimationEnd = function(){
        if (_oDiceLaunch.visible) {
            _oThis.secondAnimation();
        };
    };

    this._onDiceResultAnimationEnd = function(szAnimation){
        if(szAnimation.indexOf("stop") !== -1){
            _bAnimationOn = false;
    ...
    this.showAResult = function(iDice1, iDice2){
        _iDiceResult1 = iDice1;
        _iDiceResult2 = iDice2;
    ...
    this.unload = function () {
        _oDiceLaunch.removeAllEventListeners();
        _oDice2.removeAllEventListeners();
        s_oDices = null;
    };
